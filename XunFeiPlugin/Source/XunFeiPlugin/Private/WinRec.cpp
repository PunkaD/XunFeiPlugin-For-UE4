...
DEFINE_LOG_CATEGORY(XunFeiPluginLog);
...
void FWinRec::dbg_wave_header(WAVEHDR * buf)
{
	...
	UE_LOG(XunFeiPluginLog, Warning, TEXT("Buf %x: User= %d, Len=%d, Rec = %d, Flag = %x\n"), buf,
...
void FWinRec::data_proc(struct recorder *rec, MSG *msg)
{
	...
	UE_LOG(XunFeiPluginLog, Warning, TEXT("data....\n"));
	dbg_wave_header(buf);

	/* dwUser should be index + 1 */
	if (buf->dwUser > rec->bufcount) {
		UE_LOG(XunFeiPluginLog, Warning, TEXT("data_proc: something wrong. maybe buffer is reset.\n"));
		return;
	}
...
unsigned int  __stdcall record_thread_proc(void * para)
{
	...
	while ((bRet = GetMessageA(&msg, NULL, 0, 0)) != 0) {
		...
		switch (msg.message) {
		case MM_WIM_OPEN:
			UE_LOG(XunFeiPluginLog, Warning, TEXT("opened....\n"));
			break;
		case MM_WIM_CLOSE:
			UE_LOG(XunFeiPluginLog, Warning, TEXT("closed....\n"));
			PostQuitMessage(0);
			break;
		case MM_WIM_DATA:
