...
int FSpeechRecoginzer::sr_init(struct speech_rec * sr, const char * session_begin_params, enum sr_audsrc aud_src, int devid, struct speech_rec_notifier * notifier)
{
	...
	if (sr->session_begin_params == NULL) {
		UE_LOG(XunFeiPluginLog, Error, TEXT("mem alloc failed\n"));
		return -E_SR_NOMEM;
	}
	...
	if (aud_src == SR_MIC) {
		...
		if (sr->recorder == NULL || errcode != 0) {
			UE_LOG(XunFeiPluginLog, Error, TEXT("create recorder failed: %d\n"), errcode);
		...
		if (errcode != 0) {
			UE_LOG(XunFeiPluginLog, Error, TEXT("recorder open failed: %d\n"), errcode);
			errcode = -E_SR_RECORDFAIL;
			goto fail;
		}
	}
...
int FSpeechRecoginzer::sr_start_listening(struct speech_rec *sr)
{
	...
	if (sr->state >= SR_STATE_STARTED) {
		UE_LOG(XunFeiPluginLog, Warning, TEXT("already STARTED.\n"));
		return -E_SR_ALREADY;
	}
	...
	{
		UE_LOG(XunFeiPluginLog, Error, TEXT("\nQISRSessionBegin failed! error code:%d\n"), errcode);
		return errcode;
	}
	...
	if (sr->aud_src == SR_MIC) {
		...
		if (ret != 0) {
			UE_LOG(XunFeiPluginLog, Error, TEXT("start record failed: %d\n"), ret);
...
int FSpeechRecoginzer::sr_stop_listening(struct speech_rec *sr)
{
	...
	if (sr->state < SR_STATE_STARTED) {
		UE_LOG(XunFeiPluginLog, Warning, TEXT("Not started or already stopped.\n"));
		return 0;
	}
	...
	if (sr->aud_src == SR_MIC) {
		...
		if (ret != 0) {
			UE_LOG(XunFeiPluginLog, Error, TEXT("Stop failed! \n"));
			return -E_SR_RECORDFAIL;
		}
	...
	if (ret != 0) {
		UE_LOG(XunFeiPluginLog, Error, TEXT("write LAST_SAMPLE failed: %d\n"), ret);
