...
void on_speech_begin()
{
	...
	UE_LOG(XunFeiPluginLog, Warning, TEXT("Start Listening...\n"));
}
...
void on_speech_end(int reason)
{
	...
	{
		UE_LOG(XunFeiPluginLog, Warning, TEXT("\nSpeaking done \n"));
	}
	else
	{
...
void FXunFeiSpeech::speech_mic(const char* session_beging_params)
{
	...
	if (errcode)
	{
		UE_LOG(XunFeiPluginLog, Error, TEXT("speech recognizer init failed\n"));
		return;
	}
	// 创建识别事件状态
	...
	while (1) {
		...
		case WAIT_FAILED:
		case WAIT_TIMEOUT:
			UE_LOG(XunFeiPluginLog, Warning, TEXT("Why it happened !?\n"));
			break;
		case WAIT_OBJECT_0 + EVT_START:
			errcode = speechrecoginzer->sr_start_listening(&iat);
			if (errcode) {
				UE_LOG(XunFeiPluginLog, Error, TEXT("start listen failed %d\n"), errcode);
				isquit = 1;
			}
			break;
		case WAIT_OBJECT_0 + EVT_STOP:
			...
			if (errcode) {
				UE_LOG(XunFeiPluginLog, Error, TEXT("stop listening failed %d\n"), errcode);
				isquit = 1;
			}
			break;
